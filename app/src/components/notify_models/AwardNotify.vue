<template>
  <div class="award-notify flex flex-direction padding">
    ...
    <template
      v-if="
        (notify.params &&
          notify.params.hook_lang == 'Register' &&
          notify.params.task_target_lang == 'First') ||
        true
      "
    >
      <img class="notify-icon" src="../../assets/images/image_7@2x.png" />
      <div
        class="text-center fs-16 font-bold margin-bottom-sm"
        v-if="notify.params"
      >
        {{ $t("CONGRATULATIONS_GET_MONEY", "恭喜您获得了") }}
      </div>
      <div class="text-center fs-16 font-bold margin-bottom-sm" v-else>
        {{ $t("AWARD_HAS_ARRIVED", "奖励已到账") }}
      </div>
      <div
        class="font-bold fc-f86316 fs-18 margin-bottom-sm text-wrap text-center flex align-center justify-center"
        v-if="notify.params"
      >
        <span class=""> + </span>
      ...
      <div
        class="text-center margin-bottom-sm margin-lr-xs fs-13 font-bold fc-2acd72"
        v-if="notify.params"
      >
        <span>{{
          $t(
            "NOW_YOU_CAN_EARN_FIRST_PROFIT",
            "现在，你可以赚取你的第一笔投资收益啦"
          )
        }}</span>
      </div>
      <div class="padding-lr-sm">
        <van-button
          block
          class="font-bold margin-top breath-btn border-radius-xs fc-fff bg-2acd72"
          @click="toMoneyPage"
        >
        ...
        <van-button
          block
          class="font-bold margin-top border-radius-xs fc-fff bg-2acd72"
          plain
          @click="$emit('more')"
          v-if="notify.see_all"
        >
    ...
    <template v-else>
      ...
      <div
        class="text-center fs-16 font-bold margin-bottom-sm"
        v-html="notify.params.type_title || notify.content"
        v-if="notify.params"
      />
      <div class="text-center fs-16 font-bold margin-bottom-sm" v-else>
        {{ $t("AWARD_HAS_ARRIVED", "奖励已到账") }}
