<template>
  <div
    class="white-view index-page flex flex-direction"
  ...
  >
    ...
    >
      ...
      <p class="fs-16">
        <img
          ...
          alt=""
          style="height: 0.5rem; margin-right: 0.5rem"
        />{{ $t("HELLO", "您好，") }},<span class="fs-13">{{
          $t("NOT_LIGGED_REGISTER", "你尚未登录注册")
        }}</span>
      </p>
      <div class="flex align-center" @click="$toRouter({ name: 'Language' })">
    ...
    <div>
      ...
      <div class="flex justify-around">
        <div
          @click="$toRouter({ name: 'inviteFriend' })"
          class="margin-sm bg-purple padding border-radius vw-50 fc-fff"
        >
          {{ $t("INVITE_MONEY", "邀请赚钱") }}
        </div>
        <div class="margin-sm padding bg-pink border-radius vw-50 fc-fff">
          {{ $t("PROJECT_DESCRIPTION", "项目说明") }}
        </div>
      </div>

      <div>
        <div
          class="margin-sm bg-green padding border-radius flex justify-around"
        >
          <img
            src="../../assets/images/image_7@2x.png"
            alt=""
            style="height: 2.5rem"
          />
          <div class="fc-fff">
            ...
            <van-button
              @click="$toRouter({ name: 'inviteFriend' })"
              class="white-view border-radius-xl money fc-13b271 margin-top"
              style="height: 0.8rem"
              >{{ $t("GET_RIGHT_NOW", "立即领取") }}</van-button
            >
          </div>
        </div>
      </div>
      ...
      <ul
        class="flex justify-between vw-90 padding-top-xs padding-bottom-xs padding-left-xs padding-right-xs"
        style="margin: auto"
      >
        <li class="font-900">
          {{ $t("POPULAR_RECOMMEND", "热门充电宝推荐") }}
        </li>
        <!-- <li>更多></li> -->
      </ul>
      ...
      <div
        ...
        :key="items.id"
        class="flex flex-direction justify-around white-view vw-90 border-radius-sm margin-top-xs margin-auto"
      >
        <div class="flex justify-between margin-lr margin-tb-xs flex">
          <div class="flex">
            ...
            />
            <p
              class="fs-15 font-900 margin-left-sm"
              style="line-height: 0.8rem"
            >
              {{ $t("ORDINARY_POWERBANK", "普通充电宝") }}
            </p>
          </div>
        </div>
        ...
        <p class="fc-ff3636 text-center margin-bottom-sm">
          {{ $t("MATURITY_AUTO_RETURN", "到期自动退回租赁费") }}
        </p>
        <van-button
          @click="$toRouter({ name: 'Rent' })"
          class="bg-2acd72 border-radius-xl money margin-left-lg fc-fff vw-80 margin-bottom-sm"
          >{{ $t("RENT_TO_MAKE_MONEY", "租赁赚钱") }}</van-button
        >
      </div>
      ...
      <van-popup v-model="show_income" closeable round>
        <div class="vw-90 flex flex-direction text-center">
          ...
          <p class="fc-or fs-20">
            +523698745612<span class="fs-15 fc-default">VND</span>
          </p>
          <p class="margin fs-18">
            {{ $t("YESTERDAY_ATTIVE", "昨日收益到账") }}
          </p>
          <van-button
            @click="$toRouter({ name: 'HomeTeam' })"
            class="bg-2acd72 border-radius-xl money margin-lr fc-fff margin-bottom-sm"
            >{{ $t("EARN_MORE_REVENUE", "赚取更多收益") }}</van-button
          >
        </div>
      </van-popup>
    </div>
