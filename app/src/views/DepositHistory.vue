<template>
  <div class="deposit-history-page app-appbar flex flex-direction">
    <van-nav-bar
      :title="$t('DEPOSIT_HISTORY', '充值记录')"
      fixed
      left-arrow
    ...
    <div class="margin flex flex-direction">
      <van-row gutter="2" class="text-center margin-bottom fc-666">
        ...
        <van-col span="6" class="van-overflow-hidden text-nowrap text-ellipsis">{{ $t("DEPOSIT_WAY", "充值方式") }}</van-col>
      ...
      >
        ...
        >
          ...
          <van-col span="6" class="van-overflow-hidden text-nowrap text-ellipsis">
            <span v-if="item.recharge_type=='OnLine'">{{$t('ONLINE_PAY','在线支付')}}</span>
            <span v-else-if="item.recharge_type==''">{{$t('','')}}</span>
            <span v-else>{{item.recharge_type}}</span>

          </van-col>
          <van-col span="5" class="van-overflow-hidden text-nowrap text-ellipsis">
            <span class="fc-432af5" style="text-decoration:underline" v-if="item.order_status=='PaySuccess'">{{$t('PAY_SUCCESS','支付成功')}}</span>
            <span class="fc-ff3636" style="text-decoration:underline" v-else-if="item.order_status=='PayError'">{{$t('FAILED','失败')}}</span>
            <span class="fc-888" style="text-decoration:underline" v-else-if="item.order_status=='Paying'">{{item.recharge_type=='TransferAccounts' ? $t('IN_CHECK','审核中') : $t('PAYING','支付中') }}</span>
            <span style="text-decoration:underline" v-else>{{$t('CLOSED','已关闭')}}</span>
          </van-col>
        </van-row>
      </van-list>
    </div>
    ...
    >
      <div class="flex flex-direction position-re">
        ...
        <van-cell-group class="padding-tb custom-group-border">
          ...
          >
            <template #default>
              <span class="fc-13b271" v-if="currentOrder.order_status=='PaySuccess'">{{$t('DEPOSIT_SUCCESS','充值成功')}}</span>
              <span class="fc-432af5" v-else-if="currentOrder.order_status=='Paying'">{{$t('PAYING','支付中')}}</span>
              <span class="fc-ff3636" v-else-if="currentOrder.order_status=='PayError'">{{$t('FAILED','失败')}}</span>
              <span class="fc-888" v-else>{{$t('ORDER_CLOSED','订单已关闭')}}</span>
            </template>
          </van-cell>
          <van-cell
              :title="$t('REASON','原因')"
          ...
          />
          <van-cell
              :title="$t('DEPOSIT_WAY','充值方式')"
              :border="false"
          >
            <template #default>
              <span>{{currentOrder.recharge_type}}</span>
            </template>
          </van-cell>
          <van-cell
              :title="$t('DEPOSIT_AMOUNT','充值金额')"
              :border="false"
          >
            <template #default>
              <div class="flex align-baseline justify-end">
                <money-number
                  :class="[{'money-number':(currentOrder.wallet_type!='usdt')}]"
...
<script>
import { NavBar, List, Row, Col, Badge, Button, Empty, Popup, Icon, CellGroup, Cell } from "vant";
...
export default {
  ...
  methods: {
    closeModel() {
      this.$closeModel(this.$refs.depositHistory.$el)
      setTimeout(()=>{
    ...
    getData() {
      this.$http.get("v1/userRechargeOrderList",{
        params: {
          page: ++this.page,
          page_size: 10,
        }
